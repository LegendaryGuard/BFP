...
void CG_RocketTrail( centity_t *ent, bfpAttackSkinConfig_t *skinAtkCfg ) {
	...
	if ( contents & ( CONTENTS_WATER | CONTENTS_SLIME | CONTENTS_LAVA ) ) {
		...
		&& cg.frametime > 0.0f ) { // BFP - If paused, don't spawn bubble particles (cg_paused.integer < 1 is another solution, but not good enough for server responses)
			...
			CG_ParticleBubble( ent, cgs.media.waterBubbleShader, cgs.media.waterBubble3dShader, cgs.media.lowPolySphereModel, origin, lastPos, 900, 10, 2 );
			CG_ParticleBubble( ent, cgs.media.waterBubbleShader, cgs.media.waterBubble3dShader, cgs.media.lowPolySphereModel, origin, lastPos, 900, 10, 2 );
			CG_ParticleBubble( ent, cgs.media.waterBubbleShader, cgs.media.waterBubble3dShader, cgs.media.lowPolySphereModel, origin, lastPos, 900, 10, 2 );
...
void CG_FireWeapon( centity_t *cent ) {
	...
	// BFP - For hitscan attack type with chargeAttack or chargeAutoFire, no railTrail and no range
	if ( skinAtkCfg && wpCfg && wpCfg->attackType == ATK_HITSCAN 
	&& ( wpCfg->chargeAttack || wpCfg->chargeAutoFire )
	&& !wpCfg->railTrail && wpCfg->range <= 0 ) {
		vec3_t dir = {0, 0, 1};
		CG_ExplosionSound( cent->lerpOrigin, skinAtkCfg ); // BFP - Explosion sounds
		CG_DebrisExplosion( cent->lerpOrigin, dir, skinAtkCfg ); // BFP - Debris particles explosion
