...
void CG_KiTrail( int entityNum, vec3_t origin, qboolean remove, qhandle_t hShader ) {
	...
	if ( remove ) { // removes every segment
		cg_trails[entityNum].numSegments = ( cg_trails[entityNum].numSegments > 0 ) 
			? cg_trails[entityNum].numSegments - 1
			: 0;
	} else {
	...
	i = cg_trails[entityNum].numSegments - 1;
	while ( i > 0 ) {
	...
	i = 0;
	while ( i < cg_trails[entityNum].numSegments - 1 ) {
		j = 0;
